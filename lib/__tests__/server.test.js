import {fetchArticles} from "../renderers/api"
import serverRender from "../renderers/server"
import Api from  "../state-api/lib/index";
describe("backend rendering", () => {
    ...
    it("can fetch data from aws api gateway", async (done) => {
        let res = await fetchArticles();
        expect(res).toEqual(data);
        done();
    });

    it("can create state-api", () => {
        let api = new Api(data.data);
    })

    it("should be able to render front end app",async () => {
        const app = await serverRender();
        expect(app.initialData).toBeDefined();
        expect(app.initialMarkup).toBeDefined();
    })
});